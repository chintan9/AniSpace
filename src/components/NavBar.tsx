      ...
      <div className="bg-dark-lighten flex justify-between items-center w-screen h-12 px-one-twenty">
        <Link to="/" className="flex items-center gap-2">
          <img
            ...
            src="https://i.imgur.com/Cy7nQ7G.png"
            alt=""
          />
        ...
        <form
          ...
          className="relative h-9 md:block hidden"
        >
          <input
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            className="bg-dark-darken rounded-lg outline-none pl-3 pr-8 text-gray-300 h-full"
            type="text"
            placeholder="Search..."
          />
          <button
            className="absolute right-1 cursor-pointer top-1/2"
        ...
        {responsiveSearchActive ? (
          <Cross
            onClick={() => setResponsiveSearchActive(!responsiveSearchActive)}
        ...
        <div className="bg-dark-lighten md:hidden flex items-center w-screen h-12 px-one-twenty">
          <form onSubmit={handleFormSubmit} className="relative h-9 w-full">
            <input
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              className="bg-dark-darken rounded-lg outline-none pl-3 pr-8 text-gray-300 h-full w-full"
              type="text"
              placeholder="Search..."
            />
            <button
