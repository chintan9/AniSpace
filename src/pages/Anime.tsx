...
const Anime: FC = () => {
  ...
  return (
    <div className="relative">
      <div
        className="absolute w-screen h-48 md:h-64 top-0 left-0 background-image z-0 opacity-50"
        style={{ backgroundImage: data ? `url(${data?.banner_image})` : "" }}
      >
      ...
      <div className="flex flex-col md:flex-row gap-5 px-one-twenty pt-24 md:pt-48 pb-10 md:items-start">
        <div className="flex justify-center flex-shrink-0">
          {data ? (
            <img
              className="z-10 h-auto rounded-md"
        ...
        <div className="z-10 flex-grow">
          {data && (
            <div className="flex gap-3 md:py-3 md:justify-start justify-center md:h-16">
              ...
              {data?.trailer_url && (
                <button
                  onClick={() => setTrailerBackdropOpened(true)}
                  className="darken-btn"
                >
                  <Play style={{ height: 30, width: 30 }} />
          ...
          {data ? (
            <>
              ...
              {(data?.descriptions?.en ||
                data?.descriptions?.jp ||
                data?.descriptions?.it) && (
                <div>
                  <div
                    className={`text-gray-50 text-base overflow-hidden ${
                      !expandDescription
                        ? "max-h-36 darken-to-bottom"
                        : "max-h-infinity"
                    }`}
                  >
                    <PureText
            ...
            new Array(10)
              .fill("")
              .map((_, i) => (
                <Skeleton key={i} className="w-full h-4 my-3 rounded-md" />
              ))
          )}
        </div>
      </div>
      {trailerBackdropOpened && (
        ...
        >
          <div className="max-w-md w-full overflow-hidden">
            ...
            >
              <iframe
                onLoad={() => setTrailerSkeleton(false)}
                className={`absolute w-full h-full top-0 left-0${
                  trailerSkeleton ? " bg-gray-600 animate-pulse" : ""
                }`}
